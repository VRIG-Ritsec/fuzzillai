...
START_URL_DEFAULT = "https://en.cppreference.com/w/cpp"
ALLOWED_PATH_PREFIXES = ["/w/cpp", "/w/cpp/algorithm", "/w/cpp/memory", "/w/cpp/container", "/w/cpp/iterator", "/w/cpp/string", "/w/cpp/numeric", "/w/cpp/ranges"]
...
def crawl(start_url: str, out_dir: Path, max_pages: int, delay_s: float, user_agent: str, timeout_s: float) -> None:
    ...
    while q and (max_pages <= 0 or pages_fetched < max_pages):
        ...
        try:
            ...
            pages_fetched += 1
            print(f"Downloaded [{pages_fetched}]: {url}")
...
def default_output_dir() -> Path:
    return knowledge_docs_root() / "cpp" / "en.cppreference.com"


def main():
    parser = argparse.ArgumentParser(description="Crawl cppreference.com C++ docs into a local folder.")
    parser.add_argument("start_url", nargs="?", default=START_URL_DEFAULT, help=f"Start URL (default: {START_URL_DEFAULT})")
    parser.add_argument("--out", dest="out", default=str(default_output_dir()), help="Output directory")
    ...
    parser.add_argument("--user-agent", dest="ua", default="fuzzillai-cppreference-crawler/1.0", help="User-Agent header")
