...
START_URL_DEFAULT = "https://developer.mozilla.org/en-US/docs/Web/JavaScript"
ALLOWED_PATH_PREFIX = "/en-US/docs/Web/JavaScript"
...
def is_allowed_path(url: str) -> bool:
    return urlsplit(url).path.startswith(ALLOWED_PATH_PREFIX)
...
def default_output_dir() -> Path:
    return knowledge_docs_root() / "mdm_js"


def main():
    parser = argparse.ArgumentParser(description="Crawl MDN JavaScript docs into a local folder.")
    parser.add_argument("start_url", nargs="?", default=START_URL_DEFAULT, help=f"Start URL (default: {START_URL_DEFAULT})")
    parser.add_argument("--out", dest="out", default=str(default_output_dir()), help="Output directory (default: <repo>/mdm_js)")
    ...
    parser.add_argument("--user-agent", dest="ua", default="fuzzillai-mdn-crawler/1.0", help="User-Agent header")
