...
from RAG.paths import knowledge_docs_root
...
def normalize_url(raw_url: str) -> str:
    ...
    # normalize query params by sorting them for stable equality
    if parts[3]:
...
def path_for_url(url: str, base_out: Path) -> Path:
    ...
    if not path or path.endswith("/"):
        ...
        # If no extension, save as .html
        if not re.search(r"\.[A-Za-z0-9]{1,6}$", path):
            path = f"{path}.html"

    # Represent query uniquely if present
    query = parsed.query
    if query:
    ...
    # Mirror under netloc to avoid collisions
...
def crawl(start_url: str, out_dir: Path, max_pages: int, delay_s: float, user_agent: str, timeout_s: float) -> None:
    ...
    try:
        ...
        pass  # default to allowed if robots can't be fetched
    ...
    while q and (max_pages <= 0 or pages_fetched < max_pages):
        ...
        if not is_same_site(url, root_netloc):
            continue
        ...
        try:
            ...
            for link in extractor.links:
                ...
                if norm not in seen and is_same_site(norm, root_netloc):
                    q.append(norm)
        except Exception:
            pass
...
def default_output_dir() -> Path:
    return knowledge_docs_root() / "v8"


def main():
    parser = argparse.ArgumentParser(description="Crawl v8.dev HTML pages into a local folder.")
    parser.add_argument("start_url", nargs="?", default="https://v8.dev", help="Start URL (default: https://v8.dev)")
    parser.add_argument("--out", dest="out", default=str(default_output_dir()), help="Output directory (default: <repo>/v8)")
    parser.add_argument("--max-pages", dest="max_pages", type=int, default=0, help="Maximum pages to fetch (0 = unlimited)")
    parser.add_argument("--delay", dest="delay", type=float, default=0.2, help="Delay between requests in seconds")
    parser.add_argument("--timeout", dest="timeout", type=float, default=15.0, help="Per-request timeout in seconds")
    parser.add_argument("--user-agent", dest="ua", default="fuzzillai-v8-crawler/1.0", help="User-Agent header")
