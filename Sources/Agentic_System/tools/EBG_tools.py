...
#    """
#    Rank mutators by effectiveness based on crash discovery and coverage improvement.
#    Note: Uses execution_type and outcome as proxies since mutator tracking data is sparse.
#    
#    Args:
#        fuzzer_id (int): The ID of the fuzzer instance to analyze.
...
#    
#    Returns:
#        str: A JSON string containing effectiveness metrics grouped by execution type.
#    """
#    conn = None
#    try:
#        conn = psycopg2.connect(
...
#        debug_query = """
#        SELECT 
#            'mutator_type_id_nulls' as metric,
#            COUNT(CASE WHEN mutator_type_id IS NULL THEN 1 END) as count
#        FROM execution e
#        JOIN program p ON e.program_hash = p.program_hash
#        WHERE p.fuzzer_id = %s AND e.created_at > NOW() - INTERVAL '%s hours'
#        UNION ALL
#        SELECT 'coverage_detail_rows', COUNT(*) FROM coverage_detail
#        UNION ALL
#        SELECT 'crash_outcomes', COUNT(*) FROM execution e 
#        JOIN program p ON e.program_hash = p.program_hash
#        JOIN execution_outcome eo ON e.execution_outcome_id = eo.id
#        WHERE p.fuzzer_id = %s AND eo.outcome = 'Crashed'
#        """
#        
#        cursor.execute(debug_query, [fuzzer_id, time_window_hours, fuzzer_id])
#        debug_rows = cursor.fetchall()
#        
#        # If coverage_detail is empty or mutator data is sparse, use execution_type as proxy
#        query = """
#        SELECT 
...
#            SUM(CASE WHEN eo.outcome = 'TimedOut' THEN 1 ELSE 0 END) as timeout_count,
#            SUM(CASE WHEN eo.outcome = 'Failed' THEN 1 ELSE 0 END) as failed_count,
#            MAX(e.coverage_total) as max_coverage,
#            ROUND(AVG(e.coverage_total)::NUMERIC, 2) as avg_coverage,
#            ROUND(
...
#            ROUND(
#                (SUM(CASE WHEN eo.outcome = 'Succeeded' THEN 1 ELSE 0 END)::NUMERIC / 
#                 NULLIF(COUNT(DISTINCT e.execution_id), 0)) * 100,
#                2
...
#        result = {
#            "debug_info": debug_rows,
#            "execution_type_effectiveness": rows
#        }
#        result_json = json.dumps(result, default=json_serial, indent=2)
#        return result_json
#        
#    except psycopg2.Error as e:
#        return f"Database error: {e}... maybe try again?"
